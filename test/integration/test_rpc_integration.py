...
@pytest.mark.skip(reason="RPC integration tests require Bluetooth hardware or test mode configuration")
class TestRPCIntegration:
    """Integration tests for RPC endpoints.
    
    Note: These tests are skipped because they require actual Bluetooth hardware
    or a test mode configuration that mocks Bluetooth operations at the server level.
    The API-level tests in test/api/test_rpc.py provide sufficient coverage.
    """
    ...
    async def test_reset_volume_full_flow(self, async_api_client, db_session, admin_user):
        ...
        # Login as admin
        login_data = {"email": admin_user["email"], "password": "password123"}
        ...
        # Note: In a real integration test environment, this would mock the Bluetooth operations
        ...
        # Note: Would verify GATT operations were called in mock environment
    ...
    async def test_set_port_name_full_flow(self, async_api_client, db_session, admin_user):
        ...
        # call_args = mock_gatt.write_chars.call_args[0]
    ...
    async def test_rpc_authentication_required(self, async_api_client, db_session):
        ...
        login_data = {"email": "temp_admin@test.com", "password": "password123"}
        login_response = await async_api_client.post("/login", json=login_data)
        assert login_response.status_code == 200
